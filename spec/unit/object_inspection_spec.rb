...
RSpec.describe SuperDiff::ObjectInspection do
  describe ".inspect" do
    context "with overall_prefix and line_prefix unspecified" do
    ...
    context "with overall_prefix specified" do
      it "prepends the output with the given string" do
        inspection = described_class.inspect(
          nil,
          single_line: true,
          overall_prefix: "foo",
        )
        expect(inspection).to eq("foonil")
      end
    end

    context "with line_prefix specified" do
      it "prepends each line in the output with the given string" do
        inspection = described_class.inspect(
          SuperDiff::Test::Person.new(name: "Doc", age: 58),
          single_line: false,
          line_prefix: "foo",
